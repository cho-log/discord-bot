...
**결론**: 계정은 랜덤·교체가능하고 참가자↔계정 매핑이 없다. 참가자 56명 ≤ 계정(60→66개 예정)이라 부족하지 않다.
→ **행사 전 1회, 두 파일을 묶어 `name,username,password` 매핑 CSV를 생성**하고, 봇은 그 CSV를
이름으로 정적 조회한다. 계정이 교체가능하므로 순서대로 하나씩 짝짓고,
**남는 계정은 현장 워크인용 예비로 `임시1`, `임시2`, … (K = 계정수 − 참가자수)로 이름을 붙인다.**
운영자가 계정을 66개 발급하면 자동으로 임시 10개가 생긴다.
...
| 예비 계정 | 남는 계정은 `임시1..임시K`로 자동 이름 부여 | 현장 워크인/미등록자가 `/내계정 이름:임시3`으로 조회 |
...
- **남는 계정(계정수 − 참가자수)은 `임시1..임시K`로 이름을 붙여 예비 행으로 추가.**
- Node 내장 기능만 사용(제로 의존성). 따옴표·쉼표 포함 필드를 다루므로 최소 CSV 파서/직렬화 포함.
- 참가자 수 > 계정 수면 에러로 중단(누구는 계정 없이 남는 상황 방지).
